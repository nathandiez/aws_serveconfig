...
if [ "$USE_LOCAL_EXEC" = true ]; then
    ...
    get_ip() {
      # Method 1: Extract from terraform output directly
      ...
      ip=$(terraform refresh > /dev/null 2>&1; terraform show | grep -A 5 "public_ip" | grep -o -E '([0-9]{1,3}\.){3}[0-9]{1,3}' | grep -v '127.0.0.1' | head -n 1)
    ...
    if [ -z "$IP" ]; then
      ...
      echo "You may need to check the AWS EC2 console to see what's happening."
      exit 1
    fi
    ...
    echo "✅ EC2 instance created/configured in AWS with $DEPLOYMENT_METHOD"
